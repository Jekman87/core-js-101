...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    ...
    if (!arr.includes(elem)) return elem;
    arr.splice(i, 0, elem);
  }
  return null;
}
...
function isCreditCardNumber(ccn) {
  ...
  return !(arr.reduce((s, e) => s + +e, 0) % 10);
}
...
function isBracketsBalanced(str) {
  ...
  while (string.length) {
    newStr = string.replace(re, '');
    if (newStr === string) break;
    string = newStr;
  }
...
function getCommonDirectoryPath(pathes) {
  const newPathes = pathes.map((e) => e.split('/'));
  const resArr = [];
  for (let i = 0; i < newPathes[0].length; i += 1) {
    if (newPathes.every((e) => e[i] === newPathes[0][i])) {
      resArr.push(newPathes[0][i], '/');
    }
  }
  return resArr.join('');
}
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m1[0].length; i += 1) {
    ...
    for (let j = 0; j < m2.length; j += 1) {
      const sum = m1[i].reduce((s, e, idx) => s + e * m2[idx][j], 0);
      arrInner.push(sum);
      if (m1.length === 1) break;
    }
